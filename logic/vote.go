...
// 本项目使用简化版的投票分数
// 投一票就加 432 分  86400/200 -> 需要200张赞成票可以给你的帖子续一天 -> 《redis实战》
...
每个帖子自发表之日起一个星期之内允许用户投票，超过一个星期就不允许再投票了。
	1、到期之后将redis中保存的赞成票数及反对票数存储到mysql数据表中；
	2、到期之后删除那个 keyPostVoteZSetPF

*/
func VoteForPost(userID int64, p *modules.ParamVoteData) error {
